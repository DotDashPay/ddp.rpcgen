...
#
# This file has been modified by DotDashPay to include the function
# PROTOBUF_GENERATE_GO.
...
function(PROTOBUF_GENERATE_GO SRCS)
  ...
  foreach(FIL ${ARGN})
    ...
    set(SRC "${PROTOSRC_OUTPUT_DIR}/${FIL_WE}.pb.go")
    list(APPEND ${SRCS} ${SRC})
    list(APPEND LINK_SRCS ${SRC})

    get_filename_component(LINK_DIR ${ABS_FIL} DIRECTORY)
    list(APPEND LINK_DIRS ${LINK_DIR})
  ...
  # without the user needing to provide and explicit build rule. It
  # randomizes the target name because CMake does not allow two
  # targets with the same name and this function can be run many times
  # in the build process.
  string(RANDOM 5 "abcdefghijklmnopqrstuvwxyz" RND_TARGET)
  set(RND_TARGET_NAME "-${RND_TARGET}")
  add_custom_target("go-protos-${RND_TARGET}" ALL DEPENDS ${LINK_SRCS})

  set(INDEX 0)
  foreach(SRC ${LINK_SRCS})
    get_filename_component(ABS_FIL ${SRC} ABSOLUTE)
    get_filename_component(FIL_WE ${SRC} NAME_WE)

    list(GET LINK_DIRS INDEX LINK_DIR)
    MATH(EXPR INDEX "${INDEX}+1")

    add_custom_command(
      TARGET "go-protos-${RND_TARGET}" POST_BUILD
      COMMAND ${CMAKE_MODULE_PATH}/../bin/rewrite-go-modules.sh ${SRC}
      COMMENT "Rewriting Go protocol buffer packages in: ${SRC}")

    add_custom_command(
      TARGET "go-protos-${RND_TARGET}" POST_BUILD
      COMMAND ln -sf ${SRC} "${LINK_DIR}/${FIL_WE}.pb.go"
      COMMENT "Creating symbolic link for go protocol buffer to: ${LINK_DIR}/${FIL_WE}.pb.go")
  endforeach()
...
function(PROTOBUF_GENERATE_CPP SRCS HDRS)
  ...
  foreach(FIL ${ARGN})
    ...
    add_custom_command(
      OUTPUT "${PROTOSRC_OUTPUT_DIR}/${FIL_WE}.pb.cc"
             "${PROTOSRC_OUTPUT_DIR}/${FIL_WE}.pb.h"
      COMMAND  ${PROTOBUF_PROTOC_EXECUTABLE}
